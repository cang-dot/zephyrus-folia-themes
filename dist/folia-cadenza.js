(function(){var s=document.createElement('style');s.textContent=".folia-cadenza-root[data-v-f294dd11],.folia-classic-root[data-v-96ae650c]{background:transparent}";document.head.appendChild(s)})();var __sh__plugin_vue_export_helper_Ds65E8pU_js={};
(function(){
/**
* @vue/shared v3.5.39
...
const ne = { class: "folia-cadenza-root w-full h-full overflow-hidden pointer-events-none select-none" }, oe = { class: "absolute inset-0 w-full h-full" }, se = {
  __name: "CadenzaTheme",
  props: {
  ...
  setup(q) {
    const T = q, l = X(T, T.settings), P = a(() => l.nowTime.value), N = a(() => l.nowIndex.value), _ = a(() => l.lrcArray.value), O = a(() => _.value.map((e) => e.startTime ?? 0)), W = a(() => l.getCoverColor()), $ = a(() => W.value.primary || "#ffffff"), f = a(() => l.getClimaxState().isInClimax ? "#ff6b6b" : $.value), M = a(() => Z(_.value, O.value)), E = a(() => {
      const e = N.value;
      return e < 0 || e >= M.value.length ? null : M.value[e];
    }), S = L(null);
    let r = 0;
    const h = L([]), V = a(() => l.getConfigValue("foliaCadenzaRadius") ?? 0.25), j = a(() => l.getConfigValue("foliaGlowIntensity") ?? 1);
    function i() {
      const e = S.value;
      if (!e) {
      ...
      const n = window.devicePixelRatio || 1, s = e.clientWidth, c = e.clientHeight;
      e.width !== s * n && (e.width = s * n), e.height !== c * n && (e.height = c * n), t.setTransform(n, 0, 0, n, 0, 0), t.clearRect(0, 0, s, c);
      const x = E.value;
      if (!x) {
        h.value = [], r = requestAnimationFrame(i);
        return;
      }
      const y = P.value, u = Math.min(48, Math.max(20, s * 0.04));
      t.font = `700 ${u}px "Noto Sans SC", sans-serif`;
      const v = [], w = x.words.length, D = s / 2, G = c / 2, z = Math.min(s, c) * V.value, g = j.value;
      for (let d = 0; d < w; d++) {
        const o = x.words[d], I = -Math.PI / 2 + (w > 1 ? d / w * Math.PI * 2 : 0), C = D + Math.cos(I) * z, F = G + Math.sin(I) * z, k = Math.max(0, Math.min(1, (y - o.startTime) / Math.max(o.endTime - o.startTime, 0.3))), m = Math.min(1, k * 2), p = 0.5 + k * 0.5, B = y >= o.startTime && y <= o.endTime;
        if (B) {
          const H = u * 1.5 * g;
          t.shadowColor = f.value, t.shadowBlur = H, t.fillStyle = f.value, t.globalAlpha = Math.min(1, 0.6 + m * 0.4), t.fillText(o.text, C - t.measureText(o.text).width / 2, F + u / 3), t.shadowBlur = 0, t.globalAlpha = 1;
        }
        B ? v.push({
          text: o.text,
          style: {
      ...
      h.value = v, r = requestAnimationFrame(i);
    }
    return Y(() => {
      r = requestAnimationFrame(i);
    }), J(() => {
      ...
      R("div", oe, [
        (b(!0), A(K, null, Q(h.value, (n, s) => (b(), A("span", {
          key: s,
...
}, le = /* @__PURE__ */ U(se, [["__scopeId", "data-v-f294dd11"]]);
le.settings = [
