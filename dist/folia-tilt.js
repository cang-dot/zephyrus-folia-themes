(function(){var s=document.createElement('style');s.textContent=".folia-tilt-root[data-v-492aa817]{background:transparent} .folia-fade-enter-active[data-v-492aa817],.folia-fade-leave-active[data-v-492aa817]{transition:all .35s ease} .folia-fade-enter-from[data-v-492aa817]{opacity:0;transform:scale(.95)} .folia-fade-leave-to[data-v-492aa817]{opacity:0;transform:scale(1.05)}";document.head.appendChild(s)})();var __sh_runtime_dom_esm_bundler_69uiQWMU_js={};
(function(){
...
const Z = { class: "folia-tilt-root w-full h-full flex items-center justify-center overflow-hidden pointer-events-none select-none" }, ee = { class: "relative z-10 w-full h-[70vh] flex flex-col items-center justify-center p-8 gap-4" }, te = {
  __name: "TiltTheme",
  props: {
  ...
  setup(B) {
    const g = B, c = Y(g, g.settings), k = l(() => c.nowTime.value), M = l(() => c.nowIndex.value), C = l(() => c.lrcArray.value), z = l(() => C.value.map((e) => e.startTime ?? 0)), A = l(() => c.getCoverColor()), m = l(() => A.value.primary || "#ffffff"), w = l(() => U(m.value, 0.5)), S = l(() => Q(C.value, z.value)), f = l(() => {
      const e = M.value;
      return e < 0 || e >= S.value.length ? null : S.value[e];
    }), I = l(() => f.value?.startTime ?? 0), j = l(() => c.getConfigValue("foliaTiltSplitCount") ?? 2), D = l(() => c.getConfigValue("foliaShowTranslation") ?? !0), d = T(0), L = T(0), p = l(() => {
      const e = f.value;
      if (!e || !e.fullText) return [];
      const a = e.fullText, t = [], o = j.value;
      return N(a, o).forEach((s, u) => {
        t.push({
          chars: Array.from(s),
          isTilt: u === o - 1
        });
    ...
    function N(e, a) {
      if (a <= 1 || e.length < 6) return [e];
      const t = Math.floor(e.length / 2), o = [
        { pos: h(e, t - 2), score: 0 },
        { pos: h(e, t), score: 1 },
        { pos: h(e, t + 2), score: 2 }
      ];
      o.sort((s, u) => Math.abs(s.pos - t) - Math.abs(u.pos - t));
      const n = o[0].pos;
      return [e.slice(0, n).trim(), e.slice(n).trim()];
    }
    function h(e, a) {
      const t = [",", ".", "，", "、", "。", "；", "：", "!", "?", "！", "？", " ", "　"];
      for (const o of t) {
        const n = e.indexOf(o, Math.max(0, a));
        if (n > 0 && n < e.length - 1) return n + 1;
        const s = e.lastIndexOf(o, Math.min(e.length - 1, a));
        if (s > 0) return s + 1;
      }
      return Math.max(1, a);
    }
    $(k, (e) => {
      ...
      const t = Math.max(a.endTime - a.startTime, 0.5), o = Math.max(0, e - a.startTime);
      d.value = Math.min(o / t, 1);
    });
    function V(e, a) {
      const t = p.value.reduce((y, x) => y + x.chars.length, 0);
      if (t === 0) return { opacity: "0" };
      const n = (p.value.slice(0, e).reduce((y, x) => y + x.chars.length, 0) + a) / t, s = d.value >= n, u = n * 0.5;
      return s ? {
        opacity: "1",
        transform: "translateY(0)",
        color: e === 1 ? O.value : m.value,
        transitionDelay: `${u}s`
      } : { opacity: "0", transform: "translateY(8px)", transitionDelay: `${u}s` };
    }
    const O = l(() => c.getClimaxState().isInClimax ? "#ff6b6b" : m.value), E = T(!0), P = l(() => !D.value);
    return K(() => {
      L.value = Date.now();
    }), (e, a) => (r(), i("div", Z, [
      R("div", ee, [
        q(X, {
          ...
          default: G(() => [
            E.value && p.value.length > 0 ? (r(), i("div", {
              key: I.value,
              class: "flex flex-col items-center gap-3"
            }, [
              (r(!0), i(F, null, _(p.value, (t, o) => (r(), i("div", {
                key: o,
                class: H(["flex flex-wrap justify-center gap-[0.15em]", t.isTilt ? "italic font-light" : "font-normal"]),
                style: v({
                  fontSize: t.isTilt ? "clamp(2.5rem, 7vw, 5rem)" : "clamp(1.5rem, 4vw, 3rem)",
                  letterSpacing: t.isTilt ? "0.15em" : "0.08em",
                  color: m.value
                })
              }, [
                (r(!0), i(F, null, _(t.chars, (n, s) => (r(), i("span", {
                  key: s,
      ...
      f.value?.translation && !P.value ? (r(), i("div", {
        key: 0,
      ...
      }, b(f.value.translation), 5)) : J("", !0)
    ]));
  }
}, ae = /* @__PURE__ */ W(te, [["__scopeId", "data-v-492aa817"]]);
ae.settings = [
