(function(){var s=document.createElement('style');s.textContent=".folia-cadenza-root[data-v-f294dd11],.folia-classic-root[data-v-96ae650c]{background:transparent} .folia-ripple[data-v-96ae650c]{animation:folia-ripple-kf-96ae650c .5s ease-out forwards} @keyframes folia-ripple-kf-96ae650c{0%{transform:translate(-50%,-50%) scale(.2);opacity:.8}to{transform:translate(-50%,-50%) scale(1.5);opacity:0}} @keyframes folia-float-96ae650c{0%,to{transform:translateY(0)}50%{transform:translateY(-14px)}} .folia-fade-enter-active[data-v-96ae650c],.folia-fade-leave-active[data-v-96ae650c]{transition:all .3s ease} .folia-fade-enter-from[data-v-96ae650c]{opacity:0;filter:blur(10px);transform:scale(.9)} .folia-fade-leave-to[data-v-96ae650c]{opacity:0;filter:blur(20px);transform:scale(1.1)}";document.head.appendChild(s)})();var __sh_runtime_dom_esm_bundler_69uiQWMU_js={};
(function(){
...
**/
let ot;
const B = typeof window < "u" && window.trustedTypes;
if (B)
  try {
    ot = /* @__PURE__ */ B.createPolicy("vue", {
      createHTML: (t) => t
    });
  } catch (t) {
    process.env.NODE_ENV !== "production" && W(`Error creating trusted types policy: ${t}`);
  }
const v = "transition", D = "animation", N = /* @__PURE__ */ Symbol("_vtc"), z = {
  name: String,
  type: String,
  css: {
    type: Boolean,
  ...
  U,
  z
), st = (t) => (t.displayName = "Transition", t.props = rt, t), ft = /* @__PURE__ */ st(
  (t, { slots: e }) => Y(Z, at(t), e)
), g = (t, e = []) => {
  j(t) ? t.forEach((o) => o(...e)) : t && t(...e);
}, V = (t) => t ? j(t) ? t.some((e) => e.length > 1) : t.length > 1 : !1;
function at(t) {
  const e = {};
  for (const n in t)
    n in z || (e[n] = t[n]);
  ...
  const {
    ...
    leaveToClass: E = `${o}-leave-to`
  } = t, y = it(S), K = y && y[0], X = y && y[1], {
    onBeforeEnter: $,
    onEnter: b,
    onEnterCancelled: L,
    onLeave: w,
    ...
    onBeforeAppear: G = $,
    onAppear: J = b,
    onAppearCancelled: Q = L
  } = e, A = (n, a, C, h) => {
    n._enterCancelled = h, T(n, a ? s : d), T(n, a ? f : c), C && C();
  }, F = (n, a) => {
    n._isLeaving = !1, T(n, i), T(n, E), T(n, l), a && a();
  }, I = (n) => (a, C) => {
    const h = n ? J : b, O = () => A(a, n, C);
    g(h, [a, O]), x(() => {
      T(a, n ? m : u), p(a, n ? s : d), V(h) || M(a, r, K, O);
    });
  };
  return R(e, {
    onBeforeEnter(n) {
    ...
    onLeave(n, a) {
      n._isLeaving = !0;
      const C = () => F(n, a);
      p(n, i), n._enterCancelled ? (p(n, l), q(n)) : (q(n), p(n, l)), x(() => {
        n._isLeaving && (T(n, i), p(n, E), V(w) || M(n, r, X, C));
      }), g(w, [n, C]);
    },
    onEnterCancelled(n) {
      A(n, !1, void 0, !0), g(L, [n]);
    },
    onAppearCancelled(n) {
      A(n, !0, void 0, !0), g(Q, [n]);
    },
    onLeaveCancelled(n) {
      F(n), g(k, [n]);
    }
...
function _(t) {
  const e = et(t);
  return process.env.NODE_ENV !== "production" && nt(e, "<transition> explicit duration"), e;
}
function p(t, e) {
  e.split(/\s+/).forEach((o) => o && t.classList.add(o)), (t[N] || (t[N] = /* @__PURE__ */ new Set())).add(e);
}
function T(t, e) {
  e.split(/\s+/).forEach((r) => r && t.classList.remove(r));
  const o = t[N];
  o && (o.delete(e), o.size || (t[N] = void 0));
}
function x(t) {
...
let ct = 0;
function M(t, e, o, r) {
  const S = t._endId = ++ct, u = () => {
    S === t._endId && r();
  };
  if (o != null)
    return setTimeout(u, o);
  const { type: c, timeout: d, propCount: m } = lt(t, e);
  if (!c)
    return r();
  const f = c + "end";
  let s = 0;
  ...
  setTimeout(() => {
    s < m && i();
  }, d + 1), t.addEventListener(f, l);
}
function lt(t, e) {
  const o = window.getComputedStyle(t), r = (y) => (o[y] || "").split(", "), S = r(`${v}Delay`), u = r(`${v}Duration`), c = P(S, u), d = r(`${D}Delay`), m = r(`${D}Duration`), f = P(d, m);
  let s = null, i = 0, l = 0;
  e === v ? c > 0 && (s = v, i = c, l = u.length) : e === D ? f > 0 && (s = D, i = f, l = m.length) : (i = Math.max(c, f), s = i > 0 ? c > f ? v : D : null, l = s ? s === v ? u.length : m.length : 0);
  const E = s === v && /\b(?:transform|all)(?:,|$)/.test(
    r(`${v}Property`).toString()
  );
  return {
    type: s,
    timeout: i,
...
function P(t, e) {
  for (; t.length < e.length; )
    t = t.concat(t);
  return Math.max(...e.map((o, r) => H(o) + H(t[r])));
}
function H(t) {
...
const de = { class: "folia-classic-root w-full h-full flex items-center justify-center overflow-hidden pointer-events-none select-none" }, pe = {
  __name: "ClassicTheme",
  props: {
  ...
  setup(F) {
    const S = F, i = X(S, S.settings), $ = o(() => i.nowTime.value), L = o(() => i.nowIndex.value), C = o(() => i.lrcArray.value), A = o(() => {
      const e = [];
      for (const a of C.value)
        e.push(a.startTime ?? 0);
      return e;
    }), R = o(() => i.getCoverColor()), p = o(() => R.value.primary || "#ffffff"), m = o(() => i.getClimaxState().isInClimax ? "#ff6b6b" : p.value), T = o(() => ce(p.value, 0.5)), _ = o(() => ne(C.value, A.value)), r = o(() => {
      const e = L.value;
      return e < 0 || e >= _.value.length ? null : _.value[e];
    }), v = o(() => {
      const e = r.value;
      if (!e || e.fullText === "......") return e?.words || [];
      const a = re(e, { semantic: !0, sticky: !0 });
      return ie(a);
    }), w = b({});
    Y($, (e) => {
      const a = r.value;
      ...
      const n = a.renderHints ?? ue(a.startTime, a.endTime), t = n.wordRevealMode === "instant" ? 0.03 : n.wordRevealMode === "fast" ? 0.08 : 0.15, l = {};
      for (let s = 0; s < v.value.length; s++) {
        const u = v.value[s];
        e >= u.startTime - t && e <= u.endTime ? l[s] = "active" : e > u.endTime ? l[s] = "passed" : l[s] = "waiting";
      }
      w.value = l;
    }, { flush: "post" });
    const W = b(!0), B = o(() => !q.value), E = o(() => "clamp(2.25rem, 6vw, 4.5rem)"), I = o(() => "clamp(1.5rem, 3.5vw, 2.25rem)"), M = o(() => "clamp(1.125rem, 2.6vw, 1.25rem)");
    function j(e) {
      return () => {
        const a = Math.sin(e++) * 1e4;
        return a - Math.floor(a);
      };
    }
    function V(e, a, n) {
      const t = n.startTime, l = j(t + a), s = 20;
      return {
        x: (l() - 0.5) * s * 2,
        y: (l() - 0.5) * s * 2,
        rotate: (l() - 0.5) * 10,
    ...
    function N(e, a) {
      return !!r.value?.isChorus;
    }
    function D(e, a) {
      const n = r.value;
      if (!n) return {};
      const t = V(e, a, n), l = h(a);
      let s, u, y;
      return l === "waiting" ? (s = `translate(${t.x + Math.sin(t.y) * 50}px, ${t.y + Math.cos(t.x) * 30}px) scale(0.5) rotate(${t.rotate + 20}deg)`, u = "0", y = "all 0.4s ease") : l === "active" ? (s = `translate(${t.x}px, ${t.y}px) scale(${t.scale * 1.4}) rotate(${t.rotate}deg)`, u = "1", y = "transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.1s ease") : (s = `translate(${t.x}px, ${t.y}px) scale(${t.scale}) rotate(${t.rotate}deg)`, u = "0.82", y = "all 0.5s ease"), {
        fontSize: E.value,
    ...
    function H(e, a) {
      ...
      if (n === "active") {
        ...
        return {
          color: "transparent",
          textShadow: `0 0 ${12 * t}px ${m.value}, 0 0 ${24 * t}px ${m.value}`,
    ...
    const U = o(() => i.getConfigValue("foliaFloatSpeed") ?? 7), O = o(() => i.getConfigValue("foliaGlowIntensity") ?? 1), q = o(() => i.getConfigValue("foliaShowTranslation") ?? !0), P = o(() => "folia-ripple"), J = o(() => ({
      animation: `folia-float ${U.value}s ease-in-out infinite`
    })), K = b(1200);
    function k() {
    ...
    return Z(() => {
      ...
      x("div", {
        ...
        te(fe, {
          ...
          default: ae(() => [
            W.value && r.value && v.value.length > 0 ? (c(), f("div", {
              key: r.value.startTime,
              class: "flex flex-wrap w-full max-w-6xl content-center justify-center items-center",
              style: { perspective: "1000px", minHeight: "300px" }
            }, [
              (c(!0), f(oe, null, se(v.value, (n, t) => (c(), f("span", {
                key: `${t}-${r.value.startTime}`,
                ...
                N(n, t) ? (c(), f("span", {
                  key: 0,
                  class: le(["absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[150%] aspect-square rounded-full border pointer-events-none z-0", P.value]),
                  style: d({ borderColor: m.value })
                }, null, 6)) : z("", !0)
              ], 4))), 128))
            ])) : (c(), f("div", {
              key: "empty",
              class: "absolute opacity-50",
              style: d({ color: T.value, fontSize: I.value })
            }, " 聆听音乐... ", 4))
          ]),
          _: 1
        })
      ], 4),
      r.value?.translation && !B.value ? (c(), f("div", {
        key: 0,
        class: "absolute bottom-16 left-1/2 -translate-x-1/2 text-center z-20",
        style: d({ color: T.value, fontSize: M.value, maxWidth: "80vw" })
      }, g(r.value.translation), 5)) : z("", !0)
    ]));
  }
}, me = /* @__PURE__ */ Q(pe, [["__scopeId", "data-v-96ae650c"]]);
me.settings = [
