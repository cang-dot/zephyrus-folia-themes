...
import { createStyleContext } from '../../shared/pluginAdapter'
...
const ctx = createStyleContext(props, props.settings)
...
function getWordLayout(_word, index, line) {
...
function isChorus(_word, _index) {
  return !!activeLine.value?.isChorus
}

function getWordStyle(_word, index) {
  const line = activeLine.value
  if (!line) return {}
...
function getBodyStyle(_word, index) {
  const status = getWordStatus(index)
...
function getGlowStyle(_word, index) {
  const status = getWordStatus(index)
